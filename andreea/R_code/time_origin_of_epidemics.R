...
times_df <- data.frame(row.names = c("Str_BD", "Disc_Phylogeo", "SCOTTI"))
times_df$mean = c(mean_origin_date, as.Date("2014-01-28"), as.Date("2014-01-24"))
times_df$HPD_low = c(HPD_low_origin_date, as.Date("2013-12-26"), as.Date("2013-12-16"))
times_df$HPD_high = c(HPD_high_origin_date, as.Date("2014-02-27"), as.Date("2014-02-26"))
  
s = 20
ggplot(times_df, aes(x=rownames(times_df), y=times_df$mean)) + 
  geom_errorbar(aes(ymin=HPD_low, ymax=HPD_high), width=0.1) +
  geom_line() +
  geom_point(size=3, color=c("black", "black", "black")) + 
  xlab("Model") +
