...
data <- read.fasta("sequences200.fasta",forceDNAtolower = F)
nmes <- names(data)
...
sequences200_SCOTTI <- data

names(sequences200_SCOTTI)<- headers$ID2

setwd("C:/Users/Jeanne/Documents/ETH/Bayesian_Phylo/Ebola/Origin/SCOTTI/200sequences/run3")

#write.fasta(sequences200_SCOTTI,names(sequences200_SCOTTI),"sequences200_SCOTTI.fasta")
...
last_date <- as.numeric(chron_dates$headers.dates[200])
...
#write.csv(numeric_dates,"sequences200_dates.csv",quote = F,row.names = F, col.names = F)
...
#write.csv(hosts,"sequences200_hosts.csv",quote=F,row.names = F,col.names = F)
...
#write.csv(host_times,"sequences200_hostTimes.csv",quote=F,row.names = F)

#Column names directly removed from the Excel file

origin <- chron_dates$headers.dates[200]
mean200 <- as.Date(-179,origin)
min_HPD200 <- as.Date(-208,origin)
max_HPD200 <- as.Date(-151,origin)
time200 <-c(mean200,min_HPD200,max_HPD200)
time_range200 <- max_HPD200-min_HPD200