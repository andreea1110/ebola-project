...
chron_dates <- dates[order(dates$headers.dates),]
origin <- chron_dates$headers.dates[1]
mean50 <- as.Date(-52,origin)
min_HPD50 <- as.Date(-69,origin)
max_HPD50 <- as.Date(-40,origin)
time50 <-c(mean50,min_HPD50,max_HPD50)
time_range50 <- max_HPD50-min_HPD50

#For 200 sequences

mean200 <- as.Date(-113,origin)
max_HPD200 <-as.Date(-94,origin)
min_HPD200 <- as.Date(-135,origin)
